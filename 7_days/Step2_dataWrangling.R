...
#                                   cols = SG1:Skin7, # column names to be stored as a SINGLE variable
...
#   aes(x=forcats::fct_rev(samples), y=expression, fill=samples) +
...
# table(rowSums(myDGEList$counts==0)==7)
...
colnames(log2.cpm.df) <- c("geneID", sampleLabels)
log2.cpm.df.pivot <- pivot_longer(log2.cpm.df, # dataframe to be pivoted
                                  cols = Control1:TSLP6, # column names to be stored as a SINGLE variable
                                  names_to = "samples", # name of that new variable (column)
                                  values_to = "expression") # name of new variable (column) storing all the values (data)

log2.cpm.df.pivot$samples <- factor(log2.cpm.df.pivot$samples, levels = sampleLabels)
...
log2.cpm.filtered.df.pivot$samples <- factor(log2.cpm.filtered.df.pivot$samples, levels = sampleLabels)
...
log2.cpm.filtered.norm.df.pivot$samples <- factor(log2.cpm.filtered.norm.df.pivot$samples, levels = sampleLabels)
...
pdf("CPM_plots.pdf", height = 5, width = 15)
