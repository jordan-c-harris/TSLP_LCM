...
# myheatcolors <- bluered(75) #this is from the 'colorpanel' function in gplots (same package that heatmap.2 comes from)
# display.brewer.all()
...
# pdf("heatmap_counts.pdf", height = 8, width = 7)
#
...
#           col=rev(myheatcolors), scale='row', labRow=NA,
#           density.info="none", trace="none",
#           cexRow=1, cexCol=2, margins=c(6,2),
#           keysize = 1.5, key.par = list(cex=1),
#           key.title = NA)
#
...
#                           cols = 1:1317, # column names to be stored as a SINGLE variable
...
# modulePick_up <- 1 #use 'c()' to grab more than one cluster from the heatmap.  e.g., c(1,2)
...
#
# #choose a cluster(s) of interest by selecting the corresponding number based on the previous graph
# modulePick_down <- 2 #use 'c()' to grab more than one cluster from the heatmap.  e.g., c(1,2)
# #now we pull out the genes from this module using a fancy subsetting operation on a named vector
...
myheatcolors <- brewer.pal(name="RdBu", n=20)
clustRows <- hclust(as.dist(1-cor(t(diffGenes), method="pearson")), method="complete") #cluster rows by pearson correlation
clustColumns <- hclust(as.dist(1-cor(diffGenes, method="spearman")), method="complete")
module.assign <- cutree(clustRows, k=2)
module.color <- c("#3A53A4", "#ED2024")
module.color <- module.color[as.vector(module.assign)]
heatmap.2(diffGenes,
          Rowv=as.dendrogram(clustRows),
          Colv=FALSE,
...
genes_of_interest <- c("Gsdmc", "Nlrp10", "Nlrp12", "Bnip3", "Atg9b", "Nlrc3",
                       "Gramd4", "Aatk", "Acer2",
                      "Insrr", "Fads3", "Far1", "Acadsb", "Ffar4",
                       "Lpin2", "Acsm3","Pltp", "Agpat1", "Mfsd2a","Dgat2l6",
                      "Acox2", "Atp10d", "Abhd3","Acacb", "Hilpda", "Echdc1",
                      "Acaa2", "Idh1", "Aldh3a1", "H6pd", "Lipg", "Awat2", "Igfbp2",
                      "Agpat3", "Elovl1")

diffGenes_of_interest <- diffGenes[genes_of_interest,]
clustRows_of_interest <- hclust(as.dist(1-cor(t(diffGenes_of_interest), method="pearson")), method="complete")

## Initiate writing to PDF file
pdf("heatmap_counts_subset.pdf", height = 10, width = 4)

## Create a graphical object g here

heatmap.2(diffGenes_of_interest,
          Rowv=FALSE,
          Colv=FALSE,
...
heatmap.2(myModule_up,
          ...
          col=rev(myheatcolors), scale="row",
          density.info="none", trace="none",
          RowSideColors=module.color[module.assign%in%modulePick], margins=c(8,1))
