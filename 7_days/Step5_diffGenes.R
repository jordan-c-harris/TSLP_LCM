...
# group <- factor(targets$group, levels = c("Skin", "SG"))
...
#
# # Contrast matrix ----
# contrast.matrix <- makeContrasts(environment = SG - Skin,
#                                  levels=design)
#
...
#
# write_tsv(myTopHits.df,"myTopHits_100_genes.txt")
#
# gt(myTopHits.df)
...
#        subtitle = "Genes enriched in Sebaceous Glands, p-adj < 0.05, lfc > 1",
...
#           caption = 'Table 1: DEGs in Bulk Skin vs Sebaceous Glands',
...
fit <- lmFit(v.DEGList.filtered.norm, design)
contrast.matrix <- makeContrasts(TSLP = TSLP - Control,
                                 levels=design)
...
vplot <- ggplot(myTopHits.df) +
  ...
  #annotate("rect", xmin = 1, xmax = 12, ymin = -log10(0.01), ymax = 7.5, alpha=.2, fill="#BE684D") +
  #annotate("rect", xmin = -1, xmax = -12, ymin = -log10(0.01), ymax = 7.5, alpha=.2, fill="#2C467A") +
  labs(title="Differentially expressed genes, 7 day TSLP treatment"
       #caption=paste0("produced on ", Sys.time())) +
  ) +
  theme_classic() +
...
datatable(diffGenes.df,
          extensions = c('KeyTable', "FixedHeader"),
          caption = 'Table 1: DEGs in 7 day TSLP treated SGs, vs Control',
          options = list(keys = TRUE, searchHighlight = TRUE, pageLength = 10, lengthMenu = c("10", "25", "50", "100"))) %>%
  formatRound(columns=c(2:7), digits=2)
